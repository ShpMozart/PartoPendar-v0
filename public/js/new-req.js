...
function uploadFile(target) {
    if(target.files[0].type == 'application/pdf' && target.files[0].size <= 1000000) {
...
async function loadData() {
    let apiData = await getAPI('http://194.62.43.146:3000/api/v1/city')
...
let submitAct = document.querySelector('#submit')
    submitAct.addEventListener('click', ()=> {
        ...
        reqForm.push(newTicket)
        console.log(newTicket)

        postData('http://194.62.43.146:3000/api/v1/tickets/create', newTicket)
            .then(data => {
            console.log(data); 
        });
    })
